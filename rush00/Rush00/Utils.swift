...
//  Rush00
//
//  Created by Artem KUPRIIANETS on 20/05/2019.
//  Copyright © 2019 Jean-christophe BLONDEAU. All rights reserved.
//
...
class Utils {
	...
	static func decodeDataArray<T>(data: Data) -> [T]? {
		let tmpData = try? JSONDecoder().decode([T].self, from: data)
		return tmpData
	}
	
	static func decodeData<T>(data: Data) -> T? where T: Decodable {
		do {
			let tmpData = try JSONDecoder().decode(T.self, from: data)
			return tmpData
		} catch let err {
	...
	static func setViewProps(for view: UIView, bordWidth: CGFloat?, shColor: CGColor?, borderColor: CGColor?, shOpacity: Float?, shRadius: CGFloat?, cornerRadius: CGFloat?, shOffset: CGSize?) {
		if let bordWidth = bordWidth {view.layer.borderWidth = bordWidth }
		if let shColor = shColor { view.layer.shadowColor = shColor }
		if let shOffset = shOffset { view.layer.shadowOffset = shOffset }
		if let shOpacity = shOpacity { view.layer.shadowOpacity = shOpacity }
		if let shRadius = shRadius {view.layer.shadowRadius = shRadius }
	...
	static func alert(title: String?, message: String, prefStyle: UIAlertControllerStyle, vc: UIViewController, handler: ((UIAlertAction??) -> Void)? = nil, completion: (() -> Void)? = nil) {
        ...
        vc.present(alertMessage, animated: true, completion: completion)
    }
